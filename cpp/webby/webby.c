...
static void sha1_final(unsigned char digest[20], struct sha1 *s)
{
  ...
  for (i = 0; i < 8; ++i)
  {
    ...
    count_data[i] = (unsigned char) ( (word >> ((3 - (i & 3)) * 8)) & 0xFFU );
  }
...
void WebbyServerShutdown(struct WebbyServer *srv)
{
  ...
  {
    if (srv->connections[i].flags & WB_WEBSOCKET)
    {
      (*srv->config.ws_closed)(&srv->connections[i].public_data);
    }
...
static void reset_connection(struct WebbyServer *srv, struct WebbyConnectionPrv *conn, int fresh)
{
  ...
  conn->public_data.user_host_data = srv->config.user_host_data;
  if( fresh )
	  conn->public_data.user_data = 0;
...
static int wb_on_incoming(struct WebbyServer *srv)
{
  ...
  reset_connection(srv, connection, 1);
      ...
      case WBC_SERVE: {
        ...
        if (connection->flags & WB_ALIVE)
        {
          ...
          else
          {
            ...
            reset_connection(srv, connection, 0);
